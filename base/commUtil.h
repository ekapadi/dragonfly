...
class comm_error: public std::runtime_error{
  ...
  public:
    ...
    virtual ~comm_error(void) throw();
    
    comm_error(void) throw();
    comm_error(const comm_error& other) throw();
    comm_error(const std::string& msg) throw();
}; // class comm_error 
...
class abstractCommHandle{
  ...
  private:
    ...
    class buffer{
      ...
      public:
        ...
        inline void alloc(size_t size) throw();
                
        // attach to external buffer,
        //   do not transfer ownership:
        inline void attach(unsigned char *begin, unsigned char *end) throw();

        inline void free(void);

        inline void init(size_t data_size_) throw();
        
        inline void init(void) throw();

        inline size_t data_size(void)const;  // doesn't include header size

        inline void set_data_size(const size_t size) throw();
    ...
    size_t bufferedRead(void *ptr, size_t size, size_t nitems) throw();
  ...
  public:
		...
		virtual size_t read(void *ptr, size_t size, size_t nitems) throw()=0;

		virtual size_t write(const void *ptr, size_t size, size_t nitems) throw()=0;

    virtual void close(void) throw();
    
    inline static void close(abstractCommHandle*& h) throw();
    ...
    bool buffered(bool flag, size_t bytes=10*1024*1024) throw();    
...
// global methods:
inline size_t read(void *ptr, size_t size, size_t nitems, abstractCommHandle* h) throw();
inline size_t write(const void *ptr, size_t size, size_t nitems, abstractCommHandle* h) throw();
inline void close(abstractCommHandle*& h) throw();
...
class fileHandle: public abstractCommHandle{
  ...
  public:
    ...
    int fd(void)const { return fd_; }
		...
		// for the moment, exact match to FILE signatures:
		virtual size_t read(void *ptr, size_t size, size_t nitems) throw();

		virtual size_t write(const void *ptr, size_t size, size_t nitems) throw();
    ...
    static fileHandle* open(const char *pathname, int flags, mode_t mode) throw();
    ...
    // file exists:
		...
		// file size:
		static size_t sizeOnMedia(const char *fileName) throw();
...
// "fopen" signature and behavior: 
inline abstractCommHandle* open(const char *filename, const char *mode) throw();
// "open" signature and behavior:
inline abstractCommHandle* open(const char *pathname, int flags, mode_t mode) throw();
...
class processCommHandle: public abstractCommHandle{
  ...
  public:    
    ...
    // ------ MPI collective communication: --------
		...
		// gather to all ranks:
		template <class U>
		void all_gather(const U& u, std::vector<U>& vU) throw();
    ...
    // allocate on heap, allow only modes that make sense for processes:
		static processCommHandle* open(const int rank, const MPI::Comm& comm, const char *mode, 
                                    int tag=0, TRANSFER_KIND transfer_type=POINT2POINT) throw();

    static void attachMPISendBuffer(size_t bufSize=32*1024*1024) throw();
